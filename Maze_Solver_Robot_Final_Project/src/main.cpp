...
void loop() 
{
    ...
    if(currentMicros - previousMicros >= interval)
    {
        ...
        MOV_calc_next_state(MOV, WALL, distance_front*100, distance_left*100, distance_right*100);
        WALL_calc_next_state(WALL, distance_front*100, distance_left*100, distance_right*100);

        /* UPDATE STATES */
        set_state(MOV, MOV.state_new);
        set_state(WALL, WALL.state_new);

        /* OUTPUTS FROM THE CURRENT STATES */
        MOV_calc_outputs(MOV, distance_front*100, distance_right*100, distance_left*100);
